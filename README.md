...
Project Overview
Build a two-app financial awareness ecosystem called FinSight. The project consists of two separate React + Vite applications that share a single Firebase Firestore database in real-time. App one is a PhonePe payment simulator. App two is the FinSight intelligence dashboard. When a user makes a payment in the PhonePe app, the FinSight dashboard updates instantly — live score change, live chart update, live AI alert. This real-time cross-app sync is the core demo moment.

Tech Stack
React with Vite for both apps. Tailwind CSS for styling. Firebase Firestore for real-time database. Google Gemini API for all AI features. Chart.js for pie and line charts. jsPDF for weekly report export. Host both apps on Vercel.
...
Single scrollable page. Dark theme — background #0a0a0a, cards #1a1a2e, accent blue-purple gradient. All data reads from the same Firebase instance as PhonePe in real-time using onSnapshot listeners.
Build the page in this exact top to bottom section order.
Section 1 — Stats Row shows three cards side by side. Total Spent This Month reads sum of all transactions this month from Firebase. Monthly Budget Left shows monthly budget minus total spent. Average Daily Spend shows total spent divided by days elapsed this month. Budget is editable inline — when user changes it, update Firebase budget/current document which PhonePe also reads.
...
Section 4 — Charts two cards side by side. Left card is a donut pie chart using Chart.js showing category-wise spending percentage for current month. Right card is a line chart showing daily total spend for the last 7 days. Both update live when new Firebase transaction arrives.
...
Firebase Schema
Transactions collection stores each payment with amount, category, name, type as upi or cash, needOrWant as need or want, gst as optional string, and timestamp as Unix milliseconds.
Budget collection has a single document called current with fields monthly as number and updatedAt as timestamp.
EMIs collection stores each EMI with name, amount, and dueDate as day of month number.
Goals collection stores each goal with name, targetAmount, and savedAmount.
Streak collection stores one document per date with fields date as YYYY-MM-DD string, budgetForDay, spentForDay, and status as green, yellow, orange, or red.
Marketplace collection stores each listing with type, brand, originalValue, askingPrice, platformFee, sellerNote, status, and timestamp.
